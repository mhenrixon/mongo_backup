...
if [ ! -z "$CONFIG" ] ; then
  . $CONFIG
elif [ -f "mongo_backup.conf" ]; then
  . ./mongo_backup.conf
else
  logger -s "mongo_backup unable to read the configuration file, exiting prematurely"
  exit 1
fi
...
function error_trap {
  ...
  ERROR_RETURN=$3
  COMMAND_RETURN=$4
  logger "mongo_backup - error in function - $FUNC_RETURN - line $LINE_RETURN - error $ERROR_RETURN"
...
if [ -f $PID_FILE ] ; then
  ...
  logger "mongo_backup - exiting prematurely, pid file exists @ $PID_FILE"
  echo -e "mongo_backup - exiting prematurely, pid file exists @ $PID_FILE" | tee -a $LOG_FILE
  error_trap
fi  
...
function prepare_job {
  log "mongo_backup - ALIVE - preparing mongo backup job..."
...
function perform_backup {
  ...
  if [ "$DO_BACKUP" == "full" ] ; then
    ...
    if [ $RETURN -ne 0 ] ; then
      ...
      log "mongo_backup terminating early" 
      error_trap
    fi
...
function send_mail {
  ...
  if [ $EMAIL_REPORT -eq "1" ] ; then
    ...
    EMAIL_SUBJECT="mongo_backup - success - host:  $MB_HOST : size:  $MB_SIZE : runtime:  $TIME_DIFF minutes"
    MAILER="$(which mail)"
      if [ ! -f  "$MAILER" ] ; then
